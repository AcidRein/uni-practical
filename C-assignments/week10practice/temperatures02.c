...
void reallocateArray(float* moveFrom, float* moveTo, int moveFromSize) { // Moves all elements in param1 to param2 array
...
float* doubleSizeArray(float* fullArray, int newArraySize) {
    int oldarraySize = newArraySize / 2; // store the size fo the old 'full' array
    float* newArray = malloc(newArraySize * sizeof(float)); // create new array with double original size
    reallocateArray(fullArray, newArray, oldarraySize); // move old array element to new array
    // Free up old array
    free(fullArray);
...
int main(void) {
    ...
    while (1) {
        ...
        *(arrayPtr + currLocation) = tempValue; 
        ...
        if (currLocation == arrSize) { // if the current Pointer location is at the end of the array,
            arrSize *= 2; // double array size
            // create a new array and move all elements to new array
            arrayPtr = doubleSizeArray(arrayPtr, arrSize); // use the new array to set as previous
        }
    }
